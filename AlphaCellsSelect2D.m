...
switch alpha_select_option
   ...
   otherwise
      ...
      figure('name','Edges selected from ONE end only.')
      ...
      alpha_plot1 = plot(X,Y,'b');
      title('This is the selected complex. See command window.')
...
%       E = unique(TT,'rows');
%       % plot the 1 skeleton of the alpha complex so the user can see the result
%       figure('name','Edges selected from BOTH ends.')
%       P1 = plot(DT.X(GoodIndex,1),DT.X(GoodIndex,2),'k.'); % raw data
%       axis equal
%       hold on
%       axis tight
%       X=[DT.X(E(:,1),1)';DT.X(E(:,2),1)'];
%       Y=[DT.X(E(:,1),2)';DT.X(E(:,2),2)'];
%       alpha_plot2 = plot(X,Y,'b');
%       title('This is NOT the selected complex.')

      
      
%       fprintf('\n\n')
%       fprintf('\n\n        FUNCTION TERMINATED!!!\n\n')
%       fprintf('Our selection scheme does not yet choose which edges to hop on.\n')
%       error('Selection scheme is incomplete.  Hop cannot run.')
end

if ~isempty(GoodEdges) && alpha_select_option ~= 3
   % plot the 1 skeleton of the alpha complex so the user can see the result
   ...
   title('See command window to HOP on this complex.')
end
end % main function